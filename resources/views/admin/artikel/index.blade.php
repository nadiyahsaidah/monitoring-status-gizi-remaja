...
@section('content')
    @if (Auth::user()->role != 'remaja')
                                ...
                                <tbody>
                                    @foreach ($artikels as $artikel)
                                        <tr>
                                            ...
                                            <td>{{ $artikel->judul }}</td>
                                            <td>{{ \Illuminate\Support\Str::limit($artikel->deskripsi, 150, '...') }}</td>
                                            <td><img src="{{ asset('storage/' . $artikel->gambar) }}" width="100" alt=""></td>
                                            <td>
                                                <div class="d-flex">
        ...
        <div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form action="{{ route('artikel.update', 0) }}" method="POST" id="editForm" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="modal-header">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.editBtn').click(function() {
                ...
                $('#editForm').attr('action', '{{ route('artikel.update', ':id') }}'.replace(':id', id));
